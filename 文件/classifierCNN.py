...
def classify_model_with_preprocessed_data_CNN( batch_size=64):
    ...
    model = CNN().to(device)
    model.load_state_dict(torch.load('CNN_ATT-esm+anova1.pth', map_location=device))
    model.eval()
    ...
    result_df.to_csv('GUI/0-CNN-ATT-esm+anova1结果.csv', index=False)
    ...
    # 将DataFrame保存为CSV文件
    metrics_df.to_csv('GUI/0-CNN-ATT-esm+anova1结果指标.csv', index=False)
